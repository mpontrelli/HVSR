...
for i = 1:a
    ...
    plot(time,yn,'b')
    hold on
    area(time,yp,'Facecolor','k')
    %plot(sta)
    
    if i ==1
        xlabel('time (ms)','FontName', 'Times New Roman', 'FontSize', 18);
    else 
        set(gca,'xtick',[], 'ytick', [])
    end
    xlim([0,500])
    view([90, 90])
    ti = num2str(i);
    title(ti,'FontName', 'Times New Roman', 'FontSize', 18);
end
...
U=B; % my x-t domain signals defined for this script
