...
function [data] = Rosetta(filename, varargin)
    ...
    %% Zero pad on either side to equal 100000 200000 depending on fs
    if fs == 100
        len = length(V);
        f = (100000 - len)/2;
        if mod(f,1) == 0
   ...
   %% Now do response spectra
    ...
    [NS_mag_a, NS_mag_smooth_a] =  magresp_ros(NS, fs, len);
    ...
    [NS_mag, NS_mag_smooth] =  magresp_ros(v', fs, len);
    ...
    [NS_mag, NS_mag_smooth] =  magresp_ros(d', fs, len);
   ...
   [y, displacement, velocity, period, max1, max1V, max1D, per_maxA, per_maxV,...
    ...
    %% Now do magnitude response
    [EW_mag_a, EW_mag_smooth_a] =  magresp_ros(EW, fs, len);
    ...
    [EW_mag, EW_mag_smooth] =  magresp_ros(v', fs, len);
    ...
    [EW_mag, EW_mag_smooth] =  magresp_ros(d', fs, len);
    ...
    %% Now do magnitude response
    [V_mag_a, V_mag_smooth_a] =  magresp_ros(V, fs, len);
    ...
    [V_mag, V_mag_smooth] =  magresp_ros(v', fs, len);
    ...
    [V_mag, V_mag_smooth] =  magresp_ros(d', fs, len);
    ...
    data.processing.filtereddata.period_vec = period;
     ...
     %% Now do magnitude response
    [rot_mag_a, rot_mag_smooth_a] =  magresp_ros(rot, fs, len);
    ...
    [rot_mag, rot_mag_smooth] =  magresp_ros(v', fs, len);
    ...
    [rot_mag, rot_mag_smooth] =  magresp_ros(d', fs, len);
    ...
    H = NS + 1i*EW;   
    [comp_mag, comp_mag_smooth] =  magresp_ros(H, fs, len);
